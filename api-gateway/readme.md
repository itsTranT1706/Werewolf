...
- Create a test JWT (replace secret as needed): `node -e "console.log(require('jsonwebtoken').sign({userId:'demo-user'}, 'your_secret_key'))"`.
...
- Consumes `evt.broadcast` and emits `event.type` to either a room, a target user, or all sockets when neither is set.
